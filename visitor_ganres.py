...
def search():
    ...
    if option == "Название":
        mycursor.execute("SELECT * FROM coursework.genres WHERE name like '" + str(search_input) + "'")
    else:
        return
...
def draw_result():
    ...
    name=Label(result, text = "Жанр : ", font=("Minion Pro Regular",20), bd = 0, bg = "#777", fg = "#fff")
    ...
    desc=Label(result, text = "Описание : ", font=("Minion Pro Regular",20), bd = 0, bg = "#777", fg = "#fff")
    desc.place(x=100, y=120, height = 30)
    result_desc=Label(result, text = "undefined", font=("Minion Pro Regular",20), bd = 0, bg = "#777", fg = "#fff")
    result_desc.place(x=270, y=120, height = 30)
def draw_no_result():
    #result_frame
    global result
    for widget in result.winfo_children():
...
welcome_text = Label(frame2, text = "Поиск по навзванию", font=("Minion Pro Regular", 16), bg="#fbb1bd")
