...
def search():
    ...
    if search_input == "":
        ...
        mycursor.execute("SELECT * FROM coursework.titles WHERE title_id = '" + str(search_input) + "'")
    elif option == "Name":
        mycursor.execute("SELECT * FROM coursework.titles WHERE name = '" + str(search_input).upper() + "'")
    else:
        return
...
def getdata(event):
    ...
    TopicEntry.insert(0, row[2])
    DescriptionEntry.insert(END, row[3])
...
#heading label
nsec=Label(header, text="Медиа ДБ",font=("Helvetica", 28,"bold"), bg = "#23C3B8",fg="#eae2b7")
nsec.place(x=0, y=10, width=900)
#Profile frame
frame2=Frame(root, bg="#fbb1bd")
frame2.place(x=0,y=75,width=900,height=50)
welcome_text = Label(frame2, text = "Поиск по постам", font=("Minion Pro Regular", 16), bg="#fbb1bd")
...
# INSIDE LEFT BOX
leftbox_title=Label(leftbox,text="Просмотреть детали",font=("Helvetica",20,"bold"),fg="#eae2b7",bg="brown")
...
TopicLabel=Label(leftbox,text="Тема",font=("Helvetica", 15),fg="#eae2b7",bg="brown")
TopicLabel.place(x=200, y=50)
TopicEntry=Entry(leftbox,font=("Helvetica", 15),bd=0)
TopicEntry.configure(state='readonly')
TopicEntry.place(x=270, y=50, width = 200)
DescriptionLabel=Label(leftbox,text="Описание",font=("Helvetica", 15),fg="#eae2b7",bg="brown")
...
DescriptionEntry.insert(END, "Выберите запись, чтобы просмотреть детали")
DescriptionEntry.configure(state='disabled')
...
dropdown['values']=("--Search By--", "Id", "Name")
dropdown.current(0)
...
data_table=ttk.Treeview(tabfrm,columns=("title_id","genre_id","name","description","author_id","mark"),yscrollcommand=scrolly.set)
...
data_table.heading("author_id",text="author_id")
...
data_table.column("author_id",width=50)
