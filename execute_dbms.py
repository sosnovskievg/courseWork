...
def execute_query():
    ...
    try:
        ...
        messagebox.showerror("Внимание", "Ошибка, проверьте команду")
        db.rollback()
        db.close()
        return
    ...
    for widget in bottom_panel.winfo_children():
        widget.destroy()
    label1 = Label(bottom_panel, text =">>> "+ str(mycursor.rowcount)+" строк обработано", bg = "#333", fg = "#00FF00", font = ("Lucida Console", 12))
...
welcome_text = Label(frame2, text = "Выполнить команду", font=("Helvetica", 16), bg="#fbb1bd")
...
#panel_elements
searchBy=Label(panel,text="Текст команды",font=("Helvetica",12, "bold"),bg="#bbb",fg="#222")
...
queryBtn=Button(panel,text="Выполнить", command = execute_query, font=("Helvetica",16,"bold"), bd = 0)
